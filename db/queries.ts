...
export const getCoursById = cache(async (courseId: number) => {
...
export const getCourseProgress = cache(async () => {
  ...
  const firstUnCompletedLesson = unitInActiveCourse
    ...
    .find((lesson) => {
      //TODO : If something does not  works, check the last logic
      return lesson.challenges.some((challange) => {
        return (
          !challange.challengeProgress ||
          challange.challengeProgress.length === 0 ||
          challange.challengeProgress.some(
...
export const getLesson = cache(async (id?: number) => {
  ...
  const normalizedChallenges = data.challenges.map((challenge) => {
    const completed =
      //TODO : If somrthing does not works, check the last logics
      challenge.challengeProgress &&
      challenge.challengeProgress.length > 0 &&
      challenge.challengeProgress.every((progress) => progress.completed);
